...
_PUREGO_STUB_MODULE = "github.com/ebitengine/purego"
...
def _assert_go_source_compiles(source: str, tmp_path: Path) -> None:
    ...
    module_dir.mkdir()
    purego_stub_dir = module_dir / "stubs" / "purego"
    purego_stub_dir.mkdir(parents=True)

    (module_dir / "go.mod").write_text(
        (
            "module example.com/purego_gen_test\n\n"
            f"go {_GO_VERSION}\n\n"
            f"require {_PUREGO_STUB_MODULE} v0.0.0\n\n"
            f"replace {_PUREGO_STUB_MODULE} => ./stubs/purego\n"
        ),
        encoding="utf-8",
    )
    (module_dir / "generated.go").write_text(source, encoding="utf-8")
    (purego_stub_dir / "go.mod").write_text(
        f"module {_PUREGO_STUB_MODULE}\n\ngo {_GO_VERSION}\n",
        encoding="utf-8",
    )
    (purego_stub_dir / "purego.go").write_text(
        "package purego\n\n"
