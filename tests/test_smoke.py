...
def test_rejects_subcommand_like_argument() -> None:
    ...
    result = _run_cli("generate", "--lib-id", _FIXTURE_LIB_ID, "--header", str(_PRIMARY_HEADER))
    assert result.returncode == _ARGPARSE_USAGE_ERROR
    assert "unrecognized arguments: generate" in result.stderr


def test_generates_output_for_valid_input() -> None:
    """Valid inputs should emit generated source to stdout."""
    result = _run_cli(
        "--lib-id",
        _FIXTURE_LIB_ID,
        "--header",
    ...
    assert result.returncode == 0
    assert result.stdout.startswith(_GENERATED_HEADER_PREFIX)
