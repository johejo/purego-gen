...
func consumeString(value CXString) string {
...
func TestGeneratedBindingsParseHeaderWithLibclang(t *testing.T) {
	...
	if err := clang_register_functions(handle); err != nil {
	...
	t.Cleanup(func() {
		clang_disposeTranslationUnit(translationUnitWithoutDefine)
	})
	if got := clang_getNumDiagnostics(translationUnitWithoutDefine); got == 0 {
	...
	t.Cleanup(func() {
		clang_disposeTranslationUnit(translationUnitWithDefine)
	})
	if got := clang_getNumDiagnostics(translationUnitWithDefine); got != 0 {
		t.Fatalf("clang_getNumDiagnostics() with required define = %d, want 0", got)
	}

	rootCursor := clang_getTranslationUnitCursor(translationUnitWithDefine)
	if got := clang_getCursorKind(rootCursor); got != CXCursor_TranslationUnit {
		t.Fatalf(
	...
	if got := consumeString(
		clang_getCursorKindSpelling(CXCursor_TranslationUnit),
	...
	if got := clang_getCursorKind(functionCursor); got != CXCursor_FunctionDecl {
		t.Fatalf("clang_getCursorKind(functionCursor) = %d, want %d", got, CXCursor_FunctionDecl)
	}
	if got := consumeString(clang_getCursorSpelling(functionCursor)); got != "purego_gen_stage1_make_point" {
		t.Fatalf("clang_getCursorSpelling(functionCursor) = %q, want purego_gen_stage1_make_point", got)
	}
	if got := consumeString(clang_Cursor_getRawCommentText(functionCursor)); !strings.Contains(got, "stage1 point docs") {
		t.Fatalf("clang_Cursor_getRawCommentText(functionCursor) = %q", got)
	}
	if got := clang_isCursorDefinition(functionCursor); got != 0 {
		t.Fatalf("clang_isCursorDefinition(functionCursor) = %d, want 0", got)
	}
	if got := clang_Cursor_getNumArguments(functionCursor); got != 1 {
		t.Fatalf("clang_Cursor_getNumArguments(functionCursor) = %d, want 1", got)
	}

	argCursor := clang_Cursor_getArgument(functionCursor, 0)
	if got := clang_getCursorKind(argCursor); got != CXCursor_ParmDecl {
		t.Fatalf("clang_getCursorKind(argCursor) = %d, want %d", got, CXCursor_ParmDecl)
	}
	if got := consumeString(clang_getCursorSpelling(argCursor)); got != "value" {
		t.Fatalf("clang_getCursorSpelling(argCursor) = %q, want value", got)
	}

	resultType := clang_getCursorResultType(functionCursor)
	if got := consumeString(clang_getTypeSpelling(resultType)); got != "purego_gen_stage1_point_t" {
		t.Fatalf("clang_getTypeSpelling(resultType) = %q, want purego_gen_stage1_point_t", got)
	}
	canonicalResultType := clang_getCanonicalType(resultType)
	if got := canonicalResultType.kind; got != CXType_Record {
		t.Fatalf("canonicalResultType.kind = %d, want %d", got, CXType_Record)
	}
	if got := consumeString(clang_getTypeKindSpelling(CXType_Record)); got != "Record" {
		t.Fatalf("clang_getTypeKindSpelling(CXType_Record) = %q, want Record", got)
	}
	if got := clang_Type_getSizeOf(canonicalResultType); got != 8 {
		t.Fatalf("clang_Type_getSizeOf(canonicalResultType) = %d, want 8", got)
	}
	if got := clang_Type_getAlignOf(canonicalResultType); got != 4 {
		t.Fatalf("clang_Type_getAlignOf(canonicalResultType) = %d, want 4", got)
	}

	typeDeclaration := clang_getTypeDeclaration(canonicalResultType)
	if got := clang_getCursorKind(typeDeclaration); got != CXCursor_StructDecl {
		t.Fatalf("clang_getCursorKind(typeDeclaration) = %d, want %d", got, CXCursor_StructDecl)
	}

	location := clang_getCursorLocation(functionCursor)
	var file CXFile
	var line uint32
	var column uint32
	var offset uint32
	...
	if got := consumeString(clang_getFileName(uintptr(file))); got != headerPath {
	...
	if got := clang_getCursorKind(typedefCursor); got != CXCursor_TypedefDecl {
		t.Fatalf("clang_getCursorKind(typedefCursor) = %d, want %d", got, CXCursor_TypedefDecl)
	}
	underlyingType := clang_getTypedefDeclUnderlyingType(typedefCursor)
	if got := underlyingType.kind; got != CXType_Pointer {
		t.Fatalf("underlyingType.kind = %d, want %d", got, CXType_Pointer)
	}
	if got := clang_isConstQualifiedType(clang_getPointeeType(underlyingType)); got == 0 {
	...
	if got := clang_getCursorKind(varCursor); got != CXCursor_VarDecl {
		t.Fatalf("clang_getCursorKind(varCursor) = %d, want %d", got, CXCursor_VarDecl)
	}
	if got := clang_Cursor_getStorageClass(varCursor); got != CX_SC_Extern {
		t.Fatalf("clang_Cursor_getStorageClass(varCursor) = %d, want %d", got, CX_SC_Extern)
	}
	...
	if got := clang_getCursorKind(objectMacroCursor); got != CXCursor_MacroDefinition {
		t.Fatalf(
	...
	if got := clang_Cursor_isMacroFunctionLike(objectMacroCursor); got != 0 {
		t.Fatalf("clang_Cursor_isMacroFunctionLike(objectMacroCursor) = %d, want 0", got)
	}
	if got := clang_Cursor_isMacroBuiltin(objectMacroCursor); got != 0 {
	...
	if got := clang_getTokenKind(objectMacroTokens[0]); got != CXToken_Identifier {
		t.Fatalf("clang_getTokenKind(objectMacroTokens[0]) = %d, want %d", got, CXToken_Identifier)
	}
	...
	if got := clang_getCursorKind(functionMacroCursor); got != CXCursor_MacroDefinition {
		t.Fatalf(
	...
	if got := clang_Cursor_isMacroFunctionLike(functionMacroCursor); got == 0 {
		t.Fatal("clang_Cursor_isMacroFunctionLike(functionMacroCursor) = 0, want non-zero")
	}
	if got := clang_Cursor_isMacroBuiltin(functionMacroCursor); got != 0 {
...
func mustCursorBySpelling(
	...
	file := clang_getFile(translationUnit, headerPath)
	if file == 0 {
		t.Fatalf("clang_getFile(%q) returned nil", headerPath)
	}
	location := clang_getLocation(translationUnit, file, uint32(line), uint32(column))
	cursor := clang_getCursor(translationUnit, location)
	if got := consumeString(clang_getCursorSpelling(cursor)); got != spelling {
...
func disposeTokens(translationUnit CXTranslationUnit, tokens []CXToken) {
	if len(tokens) == 0 {
		return
	}
	clang_disposeTokens(
...
func tokenSpellings(translationUnit CXTranslationUnit, tokens []CXToken) []string {
	spellings := make([]string, 0, len(tokens))
	for _, token := range tokens {
		spellings = append(spellings, consumeString(clang_getTokenSpelling(translationUnit, token)))
	}
	return spellings
}
